...
async function loadRoomDetails() {
    ...
    try {
        ...
        const response = await fetch(`http://localhost:3000/room_details_four?room=${roomNumber}`);
...
function displayStudents(room) {
    ...
    if (room.st4 && room.st4.name) {
        studentsHTML += createStudentHTML('Student 4', room.st4);
    }
    ...
    if (studentsHTML === '') {
        studentsHTML = `
            <div class="empty-slot">
                ...
                <p class="mb-0">All 4 slots are available</p>
            </div>
        `;
    }
...
async function selectRoom() {
    ...
    try {
        ...
        else if (room.st4 === null) availableSlot = 'st4';
...
// Book the room by updating the available slot
        const bookResponse = await fetch('http://localhost:3000/book_room_four', {  // ← Make sure it's book-room
            method: 'POST',
            headers: {
